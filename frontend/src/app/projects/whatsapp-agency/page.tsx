  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const liveOwnerChat = ownerData.conversations?.[0];

        // 2. Fetch other chats if in Live Mode
        let otherChats = [];
        if (isLive) {
      ...
      <div className="flex-1 flex overflow-hidden bg-[#000000]">
        ...
        {showAccessDenied && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
            ...
            >
              <div className="w-16 h-16 rounded-2xl bg-rose-500/10 flex items-center justify-center border border-rose-500/20 mx-auto mb-6">
                <ShieldAlert size={32} className="text-rose-500" />
              </div>
              <h3 className="text-xl font-bold mb-3 tracking-tight">Feature Restricted</h3>
